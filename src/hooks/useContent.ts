...
export function useContent<T = unknown>(
    ...
    const [content, setContent] = useState<T>(initialData || (DEFAULT_CONTENT as Record<string, unknown>)[key] as T);
    ...
    useEffect(() => {
        ...
        // Handshake: Request initial state from Admin Panel on mount
        if (typeof window !== 'undefined' && window.parent !== window) {
            window.parent.postMessage({ type: 'CMS_LOAD_PREVIEW_STATE' }, '*');
        }
