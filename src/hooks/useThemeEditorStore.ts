...
import { CMSData, PageContent, BlockContent } from '@/types/cms';
...
interface ThemeEditorState {
    ...
    deepUpdateField: (sectionKey: string, subSectionKey: string, fieldKey: string, value: unknown) => void;
    moveSection: (index: number, direction: 'up' | 'down') => void;
    toggleSectionVisibility: (sectionKey: string) => void;
    addNavItem: (type: 'header' | 'footer') => void;
    updateNavItem: (type: 'header' | 'footer', index: number, field: string, value: unknown) => void;
    removeNavItem: (type: 'header' | 'footer', index: number) => void;
    addFaqItem: () => void;
    updateFaqItem: (index: number, field: string, value: unknown) => void;
    removeFaqItem: (index: number) => void;
    addBlock: (page: string, type: string) => void;
    updateBlock: (page: string, blockId: string, field: string, value: unknown) => void;
...
export const useThemeEditorStore = create<ThemeEditorState>((set, get) => ({
    ...
    updateField: (sectionKey, fieldKey, value) => set((state) => ({
        cmsData: {
            ...state.cmsData,
            [sectionKey]: {
                ...((state.cmsData as unknown as Record<string, Record<string, unknown>>)?.[sectionKey] || {}),
                [fieldKey]: value
            }
        } as unknown as CMSData
    })),
    ...
    deepUpdateField: (sectionKey, subSectionKey, fieldKey, value) => set((state) => ({
        cmsData: {
            ...
            [sectionKey]: {
                ...((state.cmsData as unknown as Record<string, Record<string, unknown>>)?.[sectionKey] || {}),
                [subSectionKey]: {
                    ...((state.cmsData as unknown as Record<string, Record<string, Record<string, unknown>>>)?.[sectionKey]?.[subSectionKey] || {}),
                    [fieldKey]: value
                }
            }
        } as unknown as CMSData
    ...
    toggleSectionVisibility: (sectionKey) => set((state) => {
        ...
        if (sectionKey === 'hero') {
            newData.content_homepage = {
                ...
                hero: { ...(prev.content_homepage?.hero as unknown as Record<string, unknown>), visible: prev.content_homepage?.hero?.visible === false ? true : false }
            } as unknown as CMSData['content_homepage'];
        } else {
            ...
            if (contentKey) {
                (newData as unknown as Record<string, Record<string, unknown>>)[contentKey] = {
                    ...(prev as unknown as Record<string, Record<string, unknown>>)[contentKey],
                    visible: (prev as unknown as Record<string, Record<string, boolean>>)[contentKey]?.visible === false ? true : false
                };
            }
        }
    ...
    addFaqItem: () => set((state) => {
        ...
        const newPages = { ...(prev.content_pages || {}) } as Record<string, PageContent>;
    ...
    addBlock: (page, type) => set((state) => {
        ...
        const newPages = { ...(prev.content_pages || {}) } as Record<string, PageContent>;
        const pageData = { ...(newPages[page] || {}) } as PageContent;
        const blocks = [...(pageData.blocks || [])];

        const newBlock = {
            id: crypto.randomUUID(),
            type,
            content: (type === 'rich_text' ? { html: '' } : type === 'hero' ? { heading: '', subheading: '', image: '' } : { heading: '', text: '', image: '' }) as BlockContent
        };
        ...
        return { cmsData: { ...prev, content_pages: newPages } as unknown as CMSData };
    }),
    ...
    updateBlock: (page, blockId, field, value) => set((state) => {
        ...
        const blocks = [...(pageData.blocks || [])].map(b => {
            if (b.id === blockId) {
                return { ...b, content: { ...(b.content as Record<string, unknown>), [field]: value } as BlockContent };
            }
            return b;
        });
    ...
    loadData: async () => {
        ...
        if (!error && data) {
            ...
            data.forEach((item: { key: string; value: unknown }) => {
                ...
                (newData as unknown as Record<string, unknown>)[item.key] = val;
            });
            set({ cmsData: newData });
        }
        set({ isLoading: false });
    ...
    saveSnapshot: async (name: string) => {
        ...
        try {
            ...
            // Tier 3: Snapshot Pruning (Stay Lean)
            // Fetch all snapshots for this user/site and keep only the latest 5
            const { data: currentSnapshots } = await supabase
                .from('site_snapshots')
                .select('id')
                .order('created_at', { ascending: false });

            if (currentSnapshots && currentSnapshots.length > 5) {
                const toDelete = currentSnapshots.slice(5).map(s => s.id);
                await supabase
                    .from('site_snapshots')
                    .delete()
                    .in('id', toDelete);
