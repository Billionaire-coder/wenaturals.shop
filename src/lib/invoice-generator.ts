...
export const generateInvoicePDF = async (order: Order) => {
    ...
    try {
        ...
        try {
            ...
            (doc as unknown as { setGState: (state: unknown) => void; GState: new (config: unknown) => unknown }).setGState(new (doc as unknown as { GState: new (config: unknown) => unknown }).GState({ opacity: 0.1 })); // 10% opacity for watermark           const wmWidth = 150;
            const wmWidth = 150;
            const wmHeight = 150;
    ...
    order.items.forEach((item: OrderItem) => {
        ...
        const splitName = doc.splitTextToSize(name, 90); // Wrap at 90 units
