...
import {
    ...
    Heading1, Heading2, Heading3, Minus, Video, Palette
} from "lucide-react";
...
declare global {
    interface Window {
        cloudinary?: {
            createUploadWidget: (options: Record<string, unknown>, callback: (error: Error | null, result: CloudinaryResult) => void) => CloudinaryWidget;
        };
    }
}
        ...
        <div className="flex flex-wrap gap-1 p-2 border-b border-white/5 bg-white/[0.02] sticky top-0 z-10 items-center">
            ...
            <div className="relative group/color-picker flex items-center">
                <button
                    type="button"
                    title="Font Color"
                    className="p-1.5 rounded-md transition-all hover:bg-white/10 text-zinc-500 hover:text-zinc-300 flex items-center gap-1"
                >
                    <Palette className="w-4 h-4" />
                </button>
                <div className="absolute top-full left-0 pt-2 z-50 hidden group-hover/color-picker:block">
                    <div className="bg-zinc-900 border border-white/10 p-2 rounded-xl shadow-2xl flex gap-2">
                        {alchemicalColors.map((c) => (
                            <button
                                key={c.color}
                                type="button"
                        ...
                        <button
                            ...
                            title="Reset Color"
                            className="w-6 h-6 rounded-full border border-white/10 flex items-center justify-center hover:scale-110 transition-transform bg-white/5 text-[10px] font-bold"
                        >
                            ×
                        </button>
                    </div>
