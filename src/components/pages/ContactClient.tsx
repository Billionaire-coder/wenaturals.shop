...
export function ContactClient({ initialContent }: { initialContent?: Record<string, unknown> }) {
    const content = useContent<Record<string, ContactContent>>('content_pages', initialContent ? { [typeof initialContent === 'object' ? 'content_pages' : '']: initialContent as unknown as ContactContent } as Record<string, ContactContent> : undefined);
    const contactContent: ContactContent = (initialContent?.contact as ContactContent) || (content?.contact as ContactContent) || {
