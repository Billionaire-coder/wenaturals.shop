...
export function AboutClient({ initialContent }: { initialContent?: Record<string, unknown> }) {
    // Falls back to hook if initialContent is missing (e.g. during transitions)
    const content = useContent<Record<string, PageContent>>('content_pages', initialContent ? { [typeof initialContent === 'object' ? 'content_pages' : '']: initialContent as unknown as PageContent } as Record<string, PageContent> : undefined);
    const aboutContent: PageContent = (initialContent?.about as PageContent) || (content?.about as PageContent) || {
                    ...
                    <section className="py-24 px-6 bg-zinc-50 dark:bg-white/[0.02] border-y border-zinc-200 dark:border-white/5">
                        <div className="max-w-7xl mx-auto text-center">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                ...
                                ].map((card: { title: string; desc: string }, i) => (
