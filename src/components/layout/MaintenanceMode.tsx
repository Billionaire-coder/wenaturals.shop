...
export const MaintenanceMode = () => {
    const [particles, setParticles] = useState<{ x: number, y: number, scale: number, opacity: number, ax: number, ay: number, dur: number }[]>([]);
    ...
    useEffect(() => {
        const timeout = setTimeout(() => {
            const newParticles = [...Array(20)].map(() => ({
                ...
                ax: Math.random() * 2000 - 1000,
                ay: Math.random() * 2000 - 1000,
