...
import { NavigationSection } from "@/types/cms";
...
export function Footer() {
    ...
    const content = useContent<Record<string, unknown>>('content_global');
    const footerData = (content?.footer as Record<string, unknown>) || {};
    const newsletter_title = footerData.newsletter_title as string | undefined;
    const newsletter_text = footerData.newsletter_text as string | undefined;
    const copyright_text = footerData.copyright_text as string | undefined;

    const navContent = useContent<NavigationSection>('content_navigation');
        ...
        <footer className="relative pt-20 md:pt-32 pb-12 px-4 sm:px-6 overflow-hidden bg-zinc-50 dark:bg-black transition-colors duration-300">
            ...
            >
                ...
                <div className="flex justify-center">
                    ...
                    >
                        <Image
                            src={((content?.footer as Record<string, unknown>)?.logo_url as string) || "/we_naturals_logo.png"}
                            alt="We Naturals"
                            fill
                        ...
                        <div className="grid grid-cols-2 gap-4 md:gap-8 w-full max-w-[200px] mx-auto lg:mx-0 lg:max-w-none text-center lg:text-left">
                            <motion.div variants={itemVariants} className="space-y-4">
                                <ul className="space-y-2 text-sm">
                                    {col1Links.map((link: { label: string; href: string }) => (
                                        <li key={link.label}>
                            ...
                            <motion.div variants={itemVariants} className="space-y-4">
                                <ul className="space-y-2 text-sm">
                                    {col2Links.map((link: { label: string; href: string }) => (
                                        <li key={link.label}>
                                            <a href={link.href} className="text-zinc-500 dark:text-zinc-400 hover:text-black dark:hover:text-white transition-colors relative group">
                                                {link.label}
                                                <span className="absolute left-0 -bottom-1 w-0 h-[1px] bg-black dark:bg-white transition-all duration-300 group-hover:w-full" />
                            ...
                            <AnimatePresence mode="wait">
                                {!showSuccess ? (
                                    ...
                                    >
                                        ...
                                        <DustParticles show={showSuccess} />
                                    </motion.div>
                                )}
                            </AnimatePresence>
                        </div>
