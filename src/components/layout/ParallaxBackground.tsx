...
import { motion, useScroll, useTransform, useSpring, useVelocity, useMotionValue, MotionValue } from "framer-motion";
...
const CausticLayer = ({ index, mouseVelocity, scrollY, color, isAnimationEnabled }: { index: number, mouseVelocity: MotionValue<number>, scrollY: MotionValue<number>, color?: string, isAnimationEnabled: boolean }) => {
...
export function ParallaxBackground() {
    ...
    const isAnimationEnabled = !performance.eco_mode && (theme.animationIntensity ?? 1) > 0.01;
        ...
        <div ref={containerRef} className="fixed inset-0 pointer-events-none overflow-hidden z-0 bg-background transition-colors duration-300">
            ...
            {performance?.parallax_enabled !== false && (
                <div className="absolute inset-0 transition-opacity duration-1000" style={{ opacity: theme.ambientOpacity }}>
                    <div
                        className={cn(
                            "absolute top-[-20%] left-[-10%] w-[80vw] h-[80vh] rounded-full blur-[150px]",
                    ...
                    <div
                        className={cn(
                            "absolute bottom-[-20%] right-[-10%] w-[70vw] h-[70vh] rounded-full blur-[150px]",
                            isAnimationEnabled && "animate-pulse"
                        )}
                        style={{ backgroundColor: `${theme.accentColor}15`, animationDelay: isAnimationEnabled ? "2s" : "0s" }}
                    />
                </div>
            )}
