...
export function Philosophy(props: { initialContent?: Record<string, unknown> }) {
    ...
    const content = (useContent('content_philosophy', props.initialContent) || {}) as Record<string, unknown>;
    if (content.visible === false) return null;

    const {
        title_top = "", title_highlight = "", title_bottom = "", description = "",
        stat_number = "", stat_text_highlight = "", stat_text = ""
    } = content as Record<string, string>;

    const media = (content.media as unknown as CMSMediaItem[]) || [];
    const points = (content.points as { title: string; description: string; visible?: boolean }[]) || [];
    ...
    return (
        <section
            ...
            className={cn(
                "py-12 md:py-20 px-4 sm:px-6 relative overflow-hidden bg-background transition-colors duration-300",
                isAdminPreview && "cursor-pointer hover:ring-4 hover:ring-blue-500/50 hover:ring-inset transition-all z-50"
            )}
        >
            {/* Background Media */}
            {(content.bg_media as unknown as CMSMediaItem[]) && (content.bg_media as unknown as CMSMediaItem[]).length > 0 && (
                <div className="absolute inset-0 z-0 opacity-5 dark:opacity-10 pointer-events-none">
                    <MediaSlider media={content.bg_media as unknown as CMSMediaItem[]} className="h-full w-full" objectFit="cover" overlay />
                </div>
            )}
                ...
                <div className="flex flex-col-reverse lg:grid lg:grid-cols-2 gap-10 md:gap-16 lg:gap-20 items-center">
                    ...
                    >
                        ...
                        <div className="space-y-8">
                            {points?.filter((point) => point.visible !== false).map((point, i) => (
