...
export function ClientRituals(props: { initialContent?: Record<string, unknown> }) {
    ...
    const content = (useContent('content_testimonials', props.initialContent) || {}) as Record<string, unknown>;
        ...
        >
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                {content.header_visible !== false && (
                    <div className="text-center mb-12">
                        <ScrollReveal direction="up">
                            <span className="text-blue-600 dark:text-blue-400 font-bold tracking-[0.4em] uppercase text-[10px] mb-4 block">{(content.subheading as string) || "The Echoes"}</span>
                        </ScrollReveal>
                        <ScrollReveal direction="up" delay={0.1}>
                            <h2 className="text-[clamp(1.75rem,8vw,4rem)] font-bold text-zinc-900 dark:text-white leading-tight">
                                {(content.heading as string) || "Client Rituals"}
                            </h2>
                        </ScrollReveal>
                    </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {items.filter((t: Testimonial) => t.visible !== false).map((t: Testimonial, i: number) => (
                        <ScrollReveal
                            key={t.author}
                            direction="up"
                            delay={0.2 + (i * 0.1)}
