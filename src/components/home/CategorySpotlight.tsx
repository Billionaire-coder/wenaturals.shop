...
export function CategorySpotlight(props: { initialContent?: Record<string, unknown> }) {
    const searchParams = useSearchParams();
    const isAdminPreview = searchParams.get('admin_preview') === 'true';

    const handleSelect = () => {
        if (isAdminPreview) {
            window.parent.postMessage({ type: 'SELECT_SECTION', section: 'categories' }, '*');
        }
    };

    const [categories, setCategories] = useState<{ title: string; tag: string; image: string; color: string }[]>([]);
    const [isHydrated, setIsHydrated] = useState(false);

    const content = (useContent('content_categories', props.initialContent) || {}) as Record<string, unknown>;
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            ...
            if (cats && products) {
                const mappedCats = cats.map((cat: { name: string }, i: number) => {
                    const prod = products.find((p: { categories: string[]; image: string }) =>
                        p.categories?.includes(cat.name) && p.image
                    );
                    return {
                        title: cat.name,
    ...
    return (
        <section
            ...
            className={cn(
                "py-12 md:py-20 px-4 sm:px-6 scroll-mt-20 relative overflow-hidden bg-background transition-colors duration-300",
                ...
                {content?.header_visible !== false && (
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 md:mb-10 gap-6">
                        <h2 className="text-3xl md:text-4xl font-bold text-zinc-900 dark:text-white leading-tight">
                            {(content?.title_start as string) || "The"} <span className="text-gradient">{(content?.title_highlight as string) || "Collections"}</span>
                        </h2>
