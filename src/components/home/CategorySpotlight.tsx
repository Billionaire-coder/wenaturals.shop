...
export function CategorySpotlight(props: { initialContent?: Record<string, unknown> }) {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            ...
            const { data: products } = await supabase.from('products').select('image, category');

            if (cats && products) {
                const mappedCats = cats.map((cat: { name: string }, i: number) => {
                    const prod = products.find((p: { category: string; image: string }) =>
                        p.category === cat.name && p.image
                    );
                    return {
                        title: cat.name,
