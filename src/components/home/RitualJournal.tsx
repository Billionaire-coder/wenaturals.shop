...
export function RitualJournal(props: { initialContent?: unknown }) {
    ...
    const content = (useContent('content_journal', props.initialContent as Record<string, unknown>) || {}) as Record<string, unknown>;
    ...
    const {
        ...
        subheading = "Insights into the alchemy of wellness...",
    ...
    return (
        <section
            ...
            className={cn(
                "py-12 md:py-20 px-4 sm:px-6 bg-background overflow-hidden relative transition-colors duration-300",
        ...
        >
            ...
            <div className="max-w-[1920px] mx-auto relative z-10">
                <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-8 max-w-7xl mx-auto">
                    <div>
                        ...
                        {content.subheading_visible !== false && (
                            <ScrollReveal direction="left" delay={0.2}>
                                <p className="text-zinc-600 dark:text-zinc-500 max-w-lg text-sm md:text-base">{subheading as string}</p>
                            </ScrollReveal>
                        )}
                    </div>
                        ...
                        <ScrollReveal direction="right" className="w-full sm:w-auto">
                            <Link href="/blog" className="block">
                                <button className="flex items-center justify-center gap-2 px-8 py-4 w-full sm:w-auto glass dark:glass rounded-full font-bold hover:bg-zinc-200 dark:hover:bg-white/10 transition-all text-zinc-900 dark:text-white border border-zinc-200 dark:border-white/10 group text-sm">
                                    {cta_text as string} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                                </button>
                            </Link>
                        </ScrollReveal>
                    )}
