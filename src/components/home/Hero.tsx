"use client";

import { useRef, useEffect, useState } from "react";
import { motion, useInView } from "framer-motion";
import { ArrowRight, Sparkles } from "lucide-react";
import { GlassCard } from "@/components/ui/GlassCard";
import { Magnetic } from "@/components/ui/Magnetic";
import { LiquidText } from "@/components/ui/LiquidText";
import { useContent } from "@/hooks/useContent";
import { MediaSlider } from "../ui/MediaSlider";
import { CMSMediaItem } from "@/types/cms";
import { useSearchParams } from "next/navigation";
import { cn } from "@/lib/utils";
import { AlchemicalSurface } from "@/components/ui/AlchemicalSurface";
import { useEnvironment } from "@/components/providers/EnvironmentalProvider";

export function Hero(props: { initialContent?: Record<string, unknown> }) {
    const { performance, isAnimationEnabled } = useEnvironment();
    const searchParams = useSearchParams();
    const [isMounted, setIsMounted] = useState(false);
    useEffect(() => {
        const timer = setTimeout(() => setIsMounted(true), 0);
        return () => clearTimeout(timer);
    }, []);

    const isAdminPreview = searchParams.get('admin_preview') === 'true';
    const handleSelect = () => {
        if (isAdminPreview) {
            window.parent.postMessage({ type: 'SELECT_SECTION', section: 'hero' }, '*');
        }
    };

    const containerRef = useRef<HTMLDivElement>(null);
    // Parallax Effects - Gated by performance setting



    const content = useContent('content_homepage', props.initialContent) as Record<string, unknown>;
    if (content?.visible === false || !isMounted) return null;

    const hero = (content?.hero || {}) as Record<string, unknown>;
    const {
        title_part_1 = "", title_part_2 = "", title_part_3 = "", title_part_4 = "",
        subtext = "", cta_primary_text = "", cta_primary_link = "",
        cta_secondary_text = "", cta_secondary_link = ""
    } = hero as Record<string, string>;

    const media = (hero.media as unknown as CMSMediaItem[]) || [];

    return (
        <section
            ref={containerRef}
            onClick={handleSelect}
            className={cn(
                "relative min-h-[100dvh] md:min-h-[110vh] flex items-center pt-16 sm:pt-32 pb-8 md:pb-10 px-4 sm:px-6 overflow-hidden perspective-1000 bg-background transition-colors duration-300",
                isAdminPreview && "cursor-pointer hover:ring-4 hover:ring-blue-500/50 hover:ring-inset transition-all z-50"
            )}
        >

            {/* Background Media / Ambient Background */}
            <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
                {(hero.bg_media as unknown as CMSMediaItem[]) && (hero.bg_media as unknown as CMSMediaItem[]).length > 0 ? (
                    <MediaSlider media={hero.bg_media as unknown as CMSMediaItem[]} className="h-full w-full" objectFit="cover" overlay priority />
                ) : (
                    /* AMBIENT AURORA BACKGROUND (Deity Level) */
                    performance?.parallax_enabled !== false && (
                        <motion.div
                            className="absolute inset-0"
                            initial={false}
                            whileInView="visible"
                            viewport={{ margin: "20%" }}
                        >
                            <motion.div
                                variants={{
                                    visible: {
                                        scale: [1, 1.3, 1],
                                        x: [0, 50, 0],
                                        y: [0, -30, 0],
                                        rotate: isAnimationEnabled ? [0, 10, 0] : 0
                                    }
                                }}
                                transition={isAnimationEnabled ? { duration: 25, repeat: Infinity, ease: "easeInOut" } : { duration: 0 }}
                                className="absolute -top-[10%] -left-[10%] w-[70vw] h-[70vw] bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-blue-600/30 to-transparent dark:from-blue-700/20 rounded-full mix-blend-screen dark:mix-blend-lighten will-change-transform"
                            />
                            <motion.div
                                variants={{
                                    visible: {
                                        scale: [1, 1.5, 1],
                                        x: [0, -40, 0],
                                        y: [0, 60, 0],
                                        rotate: isAnimationEnabled ? [0, -15, 0] : 0
                                    }
                                }}
                                transition={isAnimationEnabled ? { duration: 30, repeat: Infinity, ease: "easeInOut", delay: 2 } : { duration: 0 }}
                                className="absolute top-[20%] -right-[5%] w-[60vw] h-[60vw] bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-purple-600/30 to-transparent dark:from-purple-700/20 rounded-full mix-blend-screen dark:mix-blend-lighten will-change-transform"
                            />
                            <motion.div
                                variants={{
                                    visible: {
                                        scale: [0.8, 1.2, 0.8],
                                        x: [0, 30, 0],
                                        y: isAnimationEnabled ? [0, 40, 0] : 0
                                    }
                                }}
                                transition={isAnimationEnabled ? { duration: 20, repeat: Infinity, ease: "easeInOut", delay: 5 } : { duration: 0 }}
                                className="absolute -bottom-[20%] left-[20%] w-[50vw] h-[50vw] bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-emerald-500/30 to-transparent dark:from-emerald-600/20 rounded-full mix-blend-screen dark:mix-blend-lighten will-change-transform"
                            />
                        </motion.div>
                    )
                )}
            </div>

            <div className="max-w-7xl mx-auto px-0 sm:px-6 grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-8 items-center relative z-10 w-full">

                {/* HERO TEXT CONTENT (Left - 7 Cols) */}
                <motion.div
                    className="lg:col-span-7 order-2 lg:order-1 relative"
                    initial={false}
                >
                    {/* Floating "New Arrival" Pill */}
                    {hero.top_pill_visible !== false && (
                        <motion.div
                            initial={isAnimationEnabled ? { opacity: 0, y: 20, filter: "blur(10px)" } : false}
                            animate={{ opacity: 1, y: 0, filter: "blur(0px)" }}
                            transition={isAnimationEnabled ? { delay: 0.5, duration: 0.8 } : { duration: 0 }}
                            className="mb-8 inline-flex items-center gap-2 px-4 py-2 rounded-full border border-zinc-200 dark:border-white/10 bg-white/50 dark:bg-white/5 backdrop-blur-md shadow-sm"
                        >
                            <Sparkles className={cn("w-3 h-3 text-yellow-500 dark:text-yellow-300", isAnimationEnabled && "animate-pulse")} />
                            <span className="text-xs font-medium tracking-widest uppercase text-zinc-600 dark:text-white/80">{(hero.top_pill_text as string) || "Reimagining Nature"}</span>
                        </motion.div>
                    )}

                    <h1 className="font-medium leading-[1.1] mb-6 md:mb-8 font-display tracking-tighter text-balance text-zinc-900 dark:text-white" style={{ fontSize: 'var(--font-size-display)' }}>
                        <LiquidText text={title_part_1} delay={0.1} className="py-1 md:py-2" />
                        <div className="block">
                            <LiquidText text={title_part_2} className="text-gradient leading-[1.1] py-2 md:py-4" delay={0.4} />
                        </div>
                        <div className="flex gap-[0.2em] flex-wrap items-center py-1 md:py-2">
                            <span className="font-serif italic font-light text-zinc-500 dark:text-white/80" style={{ fontSize: 'var(--font-size-4xl)' }}>
                                <LiquidText text={title_part_3} delay={0.7} />
                            </span>
                            <LiquidText text={title_part_4} delay={0.9} />
                        </div>
                    </h1>

                    {hero.subtext_visible !== false && (
                        <motion.p
                            initial={isAnimationEnabled ? { opacity: 0, x: -10, filter: "blur(10px)" } : { opacity: 1, x: 0, filter: "blur(0px)" }}
                            animate={{ opacity: 1, x: 0, filter: "blur(0px)" }}
                            transition={{ delay: isAnimationEnabled ? 1 : 0, duration: 1.2, ease: [0.16, 1, 0.3, 1] }}
                            className="text-zinc-600 dark:text-zinc-400 max-w-xl leading-relaxed mb-8 md:mb-10 border-l-2 border-blue-500/30 dark:border-blue-400/20 pl-6"
                            style={{ fontSize: 'var(--font-size-lg)' }}
                        >
                            {subtext}
                        </motion.p>
                    )}

                    <motion.div
                        initial={isAnimationEnabled ? { opacity: 0, y: 20 } : false}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ delay: isAnimationEnabled ? 1.2 : 0, duration: 0.8 }}
                        className="flex flex-wrap gap-4"
                    >
                        {hero.cta_primary_visible !== false && (
                            <Magnetic>
                                <a href={cta_primary_link} className="block group w-full sm:w-auto">
                                    <AlchemicalSurface type="blue-essence" className="rounded-full shadow-xl shadow-blue-500/20 dark:shadow-white/10">
                                        <div className="px-6 md:px-8 py-3.5 md:py-4 font-bold text-sm tracking-wide flex items-center justify-center sm:justify-start gap-2 transition-all active:scale-95">
                                            {cta_primary_text}
                                            <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                                        </div>
                                    </AlchemicalSurface>
                                </a>
                            </Magnetic>
                        )}
                        {hero.cta_secondary_visible !== false && (
                            <Magnetic>
                                <a href={cta_secondary_link} className="px-6 md:px-8 py-3.5 md:py-4 glass dark:glass rounded-full font-bold text-sm tracking-wide hover:bg-zinc-100 dark:hover:bg-white/10 transition-all border border-zinc-200 dark:border-white/20 text-zinc-900 dark:text-white w-full sm:w-auto text-center">
                                    {cta_secondary_text}
                                </a>
                            </Magnetic>
                        )}
                    </motion.div>
                </motion.div>

                {/* HERO VISUAL (Right - 5 Cols) */}
                <motion.div
                    className="lg:col-span-5 order-1 lg:order-2 relative"
                >
                    <ViewportAwareTilt media={media} hero={hero} />
                </motion.div>
            </div>
        </section>
    );
}

function ViewportAwareTilt({ media, hero }: { media: CMSMediaItem[], hero: Record<string, unknown> }) {

    const ref = useRef<HTMLDivElement>(null);
    const isInView = useInView(ref, { margin: "0px 0px -20% 0px" });
    const gsapRef = useRef<unknown>(null);

    const { performance, isAnimationEnabled } = useEnvironment();

    useEffect(() => {
        if (isInView && !gsapRef.current && performance?.tilt_enabled !== false) {
            import("gsap").then((gsap) => {
                gsapRef.current = gsap.default;
            });
        }
    }, [isInView, performance?.tilt_enabled]);

    const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
        if (!isInView || !gsapRef.current || performance?.tilt_enabled === false) return;

        const { left, top, width, height } = e.currentTarget.getBoundingClientRect();
        const x = (e.clientX - left) / width - 0.5;
        const y = (e.clientY - top) / height - 0.5;

        (gsapRef.current as { to: (t: string, c: Record<string, unknown>) => void }).to(".hero-image", {
            rotateY: x * 20,
            rotateX: -y * 20,
            duration: 1,
            ease: "power2.out"
        });
    };

    const handleMouseLeave = () => {
        if (!gsapRef.current) return;

        (gsapRef.current as { to: (t: string, c: Record<string, unknown>) => void }).to(".hero-image", {
            rotateY: 0,
            rotateX: 0,
            duration: 1,
            ease: "elastic.out(1, 0.3)"
        });
    };

    return (
        <div
            ref={ref}
            onMouseMove={handleMouseMove}
            onMouseLeave={handleMouseLeave}
            className="hero-image relative w-full z-10 transition-transform preserve-3d cursor-pointer group"
        >
            {/* Dark Mode Aura Glow */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] bg-blue-500/10 dark:bg-blue-600/20 blur-[120px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />

            <div className="relative w-full rounded-[2rem] overflow-hidden">
                <MediaSlider media={media} objectFit="contain" priority />
            </div>

            {/* Floating Glass Elements (3D Feel - Parallaxed) */}
            {hero.purity_visible !== false && (
                <motion.div
                    className="absolute top-[10%] -right-2 lg:-right-12 z-20 translate-z-20 scale-75 md:scale-100"
                    initial={{ y: 0 }}
                    animate={isAnimationEnabled ? { y: [0, -15, 0] } : { y: 0 }}
                    transition={isAnimationEnabled ? { duration: 4, repeat: Infinity, ease: "easeInOut" } : {}}
                >
                    <GlassCard className="p-3 md:p-4 backdrop-blur-xl border-zinc-200 dark:border-white/10 shadow-xl bg-white/80 dark:bg-white/5">
                        <div className="flex items-center gap-2 md:gap-3">
                            <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-green-500/20 flex items-center justify-center text-green-600 dark:text-green-400">
                                <Sparkles className="w-4 h-4 md:w-5 md:h-5" />
                            </div>
                            <div className="flex flex-col gap-0 md:gap-1">
                                <p className="text-[8px] md:text-[10px] uppercase tracking-wider text-zinc-500 font-bold">{(hero.purity_label as string) || "Purity"}</p>
                                <p className="text-sm md:text-lg font-bold text-zinc-900 dark:text-white">{(hero.purity_value as string) || "100% Organic"}</p>
                            </div>
                        </div>
                    </GlassCard>
                </motion.div>
            )}

            {hero.result_visible !== false && (
                <motion.div
                    className="absolute bottom-[20%] -left-2 lg:-left-12 z-20 translate-z-10 scale-75 md:scale-100"
                    initial={isAnimationEnabled ? { y: 0 } : false}
                    animate={isAnimationEnabled ? { y: [0, 20, 0] } : { y: 0 }}
                    transition={isAnimationEnabled ? { duration: 5, repeat: Infinity, ease: "easeInOut", delay: 1 } : { duration: 0 }}
                >
                    <GlassCard className="p-3 md:p-4 backdrop-blur-xl border-zinc-200 dark:border-white/10 shadow-xl bg-white/80 dark:bg-white/5">
                        <div className="flex items-center gap-2 md:gap-3">
                            <div className="flex flex-col gap-0 md:gap-1">
                                <p className="text-[8px] md:text-[10px] uppercase tracking-wider text-zinc-500 font-bold">{(hero.result_label as string) || "Result"}</p>
                                <p className="text-sm md:text-lg font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600 dark:from-blue-400 dark:to-purple-400">{(hero.result_value as string) || "Clinically Proven"}</p>
                            </div>
                            <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-600 dark:text-blue-400">
                                <div className={cn("w-1.5 h-1.5 md:w-2 md:h-2 bg-blue-600 dark:bg-blue-400 rounded-full", isAnimationEnabled && "animate-ping")} />
                            </div>
                        </div>
                    </GlassCard>
                </motion.div>
            )}
        </div>
    );
}
