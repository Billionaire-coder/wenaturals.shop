...
export function GlassCard({ children, className, isPeerHovered, ...props }: GlassCardProps) {
    return (
        <motion.div
            {...(props as React.ComponentProps<typeof motion.div>)} // Cast to any to avoid complex motion type conflicts for now, or use HTMLMotionProps if we want strictness
            animate={{
