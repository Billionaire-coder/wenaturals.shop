...
export function MarqueeSimple({ initialContent }: { initialContent?: Record<string, unknown> }) {
    // Component can now be controlled via props (initialContent)
    const { text, duration, visible } = (initialContent?.marquee || initialContent || {}) as Record<string, unknown>;
    const marqueeText = (text as string) || "ORGANIC • SUSTAINABLE • CRUELTY-FREE • CLINICALLY PROVEN •";
    const marqueeDuration = (duration as number) || 20;
    const isVisible = visible !== false;

    if (!isVisible) return null;

    return (
        <div className="w-full overflow-hidden py-2 md:py-4 opacity-100 relative z-30 pointer-events-none select-none bg-transparent">
            <motion.div
            ...
            >
                {[...Array(2)].map((_, i) => (
                    <div key={i} className="flex gap-12 md:gap-32 pr-12 md:pr-32">
                        {[marqueeText, marqueeText, marqueeText].map((t: string, j: number) => (
