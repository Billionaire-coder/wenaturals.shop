...
export default function CustomCursor() {
    ...
    const magneticCacheRef = useRef<{ x: number, y: number, id: string, radius: number }[]>([]);

    const updateMagneticCache = () => {
        const elements = document.querySelectorAll('[data-magnetic]');
        const newCache: { x: number, y: number, id: string, radius: number }[] = [];
        ...
        elements.forEach((el, index) => {
            ...
            newCache.push({
                ...
                id: el.id || `magnetic-${index}`,
                radius: Math.max(rect.width, rect.height) / 2 + 50 // Dynamic attraction radius
            });
        });
        magneticCacheRef.current = newCache;
    };

    useEffect(() => {
        updateMagneticCache();
        window.addEventListener('scroll', updateMagneticCache, { passive: true });
        window.addEventListener('resize', updateMagneticCache);

        // Listen for dynamic content updates (e.g. from CMS)
        const observer = new MutationObserver(updateMagneticCache);
        observer.observe(document.body, { childList: true, subtree: true });

        return () => {
            window.removeEventListener('scroll', updateMagneticCache);
    ...
    useEffect(() => {
        const moveCursor = (e: MouseEvent) => {
            ...
            // Magnetic Logic - USE CACHE (Zero Reflow)
            let nearestTarget = null;
            let minDistance = Infinity;

            for (const target of magneticCacheRef.current) {
                const distance = Math.sqrt(Math.pow(clientX - target.x, 2) + Math.pow(clientY - target.y, 2));
            ...
            if (nearestTarget) {
                setMagneticTarget({ x: nearestTarget.x, y: nearestTarget.y });
            } else {
        ...
        const handleHoverStart = (e: MouseEvent) => {
            ...
            if (interactive) {
                const cursorType = (interactive as HTMLElement).getAttribute('data-cursor') as 'default' | 'pointer' | 'text' | 'view' | 'drag' || 'pointer';
