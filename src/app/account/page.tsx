...
import { MapPin, LogOut, Loader2, Trash2, Download } from "lucide-react";
...
export default function AccountPage() {
    ...
    const handleDeleteAddress = async (id: string) => {
        if (!confirm("Remove this location from your account?")) return;
        try {
    ...
    if (loading) return (
        <div className="min-h-screen bg-mesh flex items-center justify-center">
            <Loader2 className="w-8 h-8 animate-spin text-zinc-500 dark:text-white/50" />
        </div>
    );
        ...
        <main className="min-h-screen bg-mesh pt-20 md:pt-32 pb-12 px-4 md:px-6">
                        ...
                        <GlassCard className="p-6 md:p-8 space-y-8 relative bg-white/80 dark:bg-black/40 border-zinc-200 dark:border-white/10 shadow-xl">
                            ...
                            <div className="flex justify-between items-start">
                                <div className="space-y-1">
                                    <h2 className="text-2xl font-bold text-zinc-900 dark:text-white">Order Details</h2>
                                    <p className="text-zinc-500 dark:text-zinc-400 font-mono text-xs">#{selectedOrder.id}</p>
                            ...
                            <OrderTracker
                                ...
                                paymentStatus={selectedOrder.payment_status || "pending"}
                                isCOD={selectedOrder.payment_status === 'pending' || selectedOrder.payment_method === 'cod'}
                            />

                            {/* Product List with Images */}
                            <div className="space-y-4">
                                <h3 className="text-sm font-bold uppercase tracking-widest text-zinc-600 dark:text-zinc-500">Items</h3>
                                        ...
                                        return (
                                            <div key={item.id} className="flex gap-4 p-3 bg-zinc-50 dark:bg-white/5 rounded-xl border border-zinc-100 dark:border-transparent">
                                                ...
                                                <div className="flex-1">
                                                    <p className="font-bold text-sm text-zinc-900 dark:text-white">{item.product_name}</p>
                                                    <p className="text-xs text-zinc-500 dark:text-zinc-400">Qty: {item.quantity}</p>
                                                    <p className="text-xs text-blue-600 dark:text-blue-400 mt-1 font-bold">₹{item.price_at_purchase}</p>
                                                </div>
                                            </div>
                                        );
                                    })}
            ...
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h1 className="text-3xl font-bold text-zinc-900 dark:text-white">Your Account</h1>
                        <p className="text-zinc-600 dark:text-zinc-400">Welcome back, {user?.user_metadata?.full_name || user?.email}</p>
                    </div>
                    <button
                ...
                <div className="flex gap-4 border-b border-white/10">
                    <button
                        ...
                        className={`pb-4 text-sm font-bold uppercase tracking-widest transition-colors relative ${activeTab === 'rituals' ? 'text-zinc-900 dark:text-white' : 'text-zinc-500 dark:text-zinc-600 hover:text-zinc-700 dark:hover:text-zinc-400'}`}
                    >
                        Order History
                ...
                <div className="min-h-[400px]">
                    {activeTab === 'rituals' ? (
                        <div className="space-y-4 max-w-2xl">
                            {orders.length === 0 ? (
                                <div className="text-center py-20 text-zinc-500">No orders placed yet.</div>
                            ) : (
                                orders.map((order) => (
                                    <GlassCard
                                        key={order.id}
                                    ...
                                    >
                                                ...
                                                return (
                                                    <div key={item.id} className="flex justify-between text-sm">
                                                        <div>
                                                            ...
                                                            <p className="text-xs text-zinc-600 dark:text-zinc-500">Qty: {item.quantity}</p>
                                                        </div>
                                                        <p className="text-zinc-600 dark:text-zinc-400 font-bold">₹{item.price_at_purchase * item.quantity}</p>
                                                    </div>
                                                );
                                            })}
                                        </div>
                                        ...
                                        <div className="space-y-1 pt-1">
                                            <div className="flex justify-between text-xs text-zinc-500">
                                                <span>Subtotal</span>
                                                <span>₹{order.items.reduce((sum: number, i: OrderItem) => sum + (i.price_at_purchase * i.quantity), 0)}</span>
                                            </div>
                                            <div className="flex justify-between text-xs text-zinc-500">
                                                <span>Shipping</span>
                                                <span>Free</span>
