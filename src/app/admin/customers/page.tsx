...
import { Users, Search, MapPin, ShoppingBag, ChevronDown, ChevronUp, Phone } from "lucide-react";
...
export default function AdminCustomersPage() {
    ...
    const fetchCustomers = async () => {
        ...
        try {
            ...
            // 4. Aggregation Logic
            const aggregatedCustomers = (profiles || []).map((profile) => {
                const userOrders = (orders || []).filter((o) => o.user_id === profile.id);
                const userAddresses = (addresses || []).filter((a) => a.user_id === profile.id);

                const totalSpent = userOrders.reduce((acc: number, curr) => {
