...
interface DashboardStats {
    ...
    recentOrders: Order[];
    lowStockItems: { name: string; stock: number; id: string }[];
    salesHistory: { date: string; amount: number }[];
    categoryStats: { name: string; value: number }[];
}

const SalesPulseChart = ({ data }: { data: { date: string; amount: number }[] }) => {
    if (!data.length) return null;
    const peakValue = Math.max(...data.map(d => d.amount), 100);
    const pts = data.map((d, i) => `${(i / (data.length - 1)) * 100},${100 - (d.amount / peakValue * 80)}`).join(' ');
